...
- **Animation.** Purposeful and short. Pop (`kalimat-pop`, 220ms, `--ease-pop`) when a letter is typed; flip (`kalimat-flip`, 420ms, `--ease-in-out`) on reveal, staggered 120ms per tile right-to-left; shake (`kalimat-shake`, 420ms) on an invalid word; rise-and-fade (`kalimat-rise`) for dialogs and toasts. Default easing `--ease-out` cubic-bezier(.2,.8,.3,1). No bounce outside `--ease-pop`, no looping motion, no parallax.

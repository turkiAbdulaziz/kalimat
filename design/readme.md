...
- **Dark surface.** `[data-theme="dark"]` in `tokens/colors.css` remaps surface, text, line, accent and keyboard tokens while gameplay result colors keep one identity across themes: correct is dark brown, present is light brown, absent is taupe. Components never branch on theme — set `data-theme="dark"` on one root element and everything below follows. Two rules make that work: **never hardcode a hex or a base-ramp token (`--brown-800`) where a semantic one exists** — use `--text-on-soft` for text on `--accent-soft` and `--text-wordmark` for the brand mark — and **set `color` on the same element that carries `data-theme`**, because inherited colour is a resolved literal: a `body { color: var(--text-body) }` outside the wrapper stays light forever.

...
| `flow/` | `flow_controller.dart` (`FlowStep` signin/name/game + `flowProvider`; gating: unconfigured or `onboarded` → straight to game, existing installs migrate silently; also `displayNameProvider`, sign-out clears board+identity but **keeps stats**), `root_flow.dart` (`home:` widget — instant swaps, no route transitions). |
...
| `profile/` | `profile_screen.dart` («حسابي», pushed with zero-duration route so board state survives), `edit_name_dialog.dart` (tap the name), `save_progress_section.dart` (anonymous-only linking, moved from the old settings dialog, keeps the confirm-switch dialog), `reminder_dialog.dart` (١٢-hour steppers + ص/م). |
